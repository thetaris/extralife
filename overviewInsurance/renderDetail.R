...
renderUI({
  renderSchaden <- function(logSum){
    symbol = elTags$img(src="euro.png", width="25", border="0")
    space = tags$a(".")
  ...
  renderAbdeckung <- function(type){
    # type is 0 to 5 (best)
    return(elTags$img(src=sprintf("Signal_%i.png",type), width="25"))
  }
  
  renderStatus <- function(type=0){
    if (type==0) {
      return(elTags$img(src="Check_Icon_32.png"))
    } else {
      return(elTags$img(src="Warning_Icon_32.png"))
    }
  }
